...
class WeightParserServiceTest(unittest.TestCase):
    def test_extracts_single_and_pack_weights(self) -> None:
        ...
        self.assertAlmostEqual(extract_weight_kg_from_name("Мыло 3 шт. - 175 г"), 0.525)
        self.assertEqual(extract_weight_kg_from_name("Пюре 4 x 90 г"), 0.36)
        self.assertEqual(extract_weight_kg_from_name("Напиток 2 по 1,5 л"), 3.0)
    ...
    def test_parse_weights_dataframe_adds_expected_columns(self) -> None:
        ...
        self.assertTrue(np.isfinite(parsed.iloc[0]["Цена за кг"]))
